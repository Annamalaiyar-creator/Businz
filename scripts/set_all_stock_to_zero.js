...
async function setAllStockToZero() {
  console.log('--- Setting all stock to 0 across system ---');
  ...
  if (fs.existsSync(rawPath)) {
    ...
    console.log(`✓ Set all stock to 0 for ${rawItems.length} items in server/raw_materials_store.json`);
  }
  ...
  if (fs.existsSync(itemPath)) {
    ...
    console.log(`✓ Set all stock to 0 for ${items.length} items in server/item_store.json`);
  }
  ...
  if (fs.existsSync(prodInvPath)) {
    ...
    console.log(`✓ Set all stock to 0 for ${prodInv.length} items in server/vrm_prod_inventory.json`);
  }

  // 4. Update Supabase Cloud Database (leaves table)
  console.log('Syncing 0-stock to Supabase cloud database...');
  const pushToSupabase = async (key, data) => {
    ...
    if (records && records.length > 0) {
      ...
      console.log(`✓ Updated Supabase key ${employeeKey}`);
    } else {
      await supabase.from('leaves').insert(payload);
      console.log(`✓ Inserted Supabase key ${employeeKey}`);
    }
  };
  ...
  console.log('All stock successfully removed and reset to 0!');
}
